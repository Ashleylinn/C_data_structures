...
Quad *BST_delete(Quad *root, int tx, int ty) {
  ...
  else //key == root->key
  {
    ...
    {
      Quad *successor = find_successor(root->right); //has both children
...
int get_colour(Image *im, Quad *q) {
  ...
  for(int y = q->ty; y<q->ty + q->h; y++)
  {
    ...
    {
      number = number + im->data[x + (y * q->sx)];
      count++;
    }
  }
  return number / count;
...
Quad *split_tree(Image *im, Quad *root, int threshold) {
  ...
  Quad *one = (Quad *)calloc(1, sizeof(Quad));
  Quad *two = (Quad *)calloc(1, sizeof(Quad));
  
  if(root->wsplit == 1) //If wsplit = 1, then we split it along the width
  {
    int split_width = root->w / 2;
    one->tx = root->tx;
    one->ty = root->ty;
    one->w = split_width;
    one->h = root->h;
    ...
    two->tx = root->tx + split_width;
    two->ty = root->ty;
    two->w = root->w - split_width;
    two->h = root->h;
    two->sx = root->sx;
    two->wsplit = 0;
    two->left = NULL;
  ...
  else //If wsplit = 0, then we split along the height.
  {
    int split_height = root->h / 2;
    one->tx = root->tx;
    one->ty = root->ty;
    one->w = root->w;
    one->h = split_height;
    ...
    two->tx = root->tx; 
    two->ty = root->ty + split_height;
    two->w = root->w; 
    two->h = root->h - split_height;
    two->sx = root->sx;
    two->wsplit = 1;
    two->left = NULL;
    two->right = NULL;
  ...
  root->left = split_tree(im, one, threshold);
  root->right = split_tree(im, two, threshold);
  return root;
}
...
void save_Quad(Image *im, Quad *root) {
  ...
  int avgColor = get_colour(im, root);
  int tx = root->tx;
  int ty = root->ty;
  int h = root->h;
  int w = root->w;
